...
import Anify from '../assets/anify/anify_bg.webp';
import Anify115 from '../assets/image.webp';
import Anify114Release from '../assets/image copy.webp';
import skdevbanner from '../assets/skdev-banner.webp';
import GwallsDiscontinued from '../assets/gwalls_discontinued.webp';
  ...
  const handleShare = async (e, item) => {
    ...
    try {
      if (navigator.share) {
        ...
        setCopied(item.id);
        toast.success(`Copied "${item.title.slice(0, 30)}..." link!`);
                ...
                <div style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(item.title)}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="btn btn-secondary" style={{ fontSize: '0.85rem', padding: '0.4rem 0.6rem', color: '#1DA1F2' }} title="Share on Twitter">
                        <TwitterIcon size={14} />
                      </a>
                      <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="btn btn-secondary" style={{ fontSize: '0.85rem', padding: '0.4rem 0.6rem', color: '#0A66C2' }} title="Share on LinkedIn">
                        <LinkedinIcon size={14} />
                      </a>
                      <a href={`https://wa.me/?text=${encodeURIComponent(item.title + ' ' + window.location.href)}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="btn btn-secondary" style={{ fontSize: '0.85rem', padding: '0.4rem 0.6rem', color: '#25D366' }} title="Share on WhatsApp">
